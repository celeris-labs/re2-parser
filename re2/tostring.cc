  ...
  switch (re->op()) {
    ...
    case kRegexpAlternate:
      ...
      if ((*t_)[t_->size()-1] == '|')
        ...
        throw std::runtime_error(std::string("Bad final char: ") + *t_);
      if (prec < PrecAlternate)
        t_->append(")");
      break;
    ...
    case kRegexpRepeat:
      if (re->max() == -1)
        AppendFormat(t_, "{%d,}", re->min());
      else if (re->min() == re->max())
        AppendFormat(t_, "{%d}", re->min());
      else
        AppendFormat(t_, "{%d,%d}", re->min(), re->max());
      if (re->parse_flags() & Regexp::NonGreedy)
        t_->append("?");
      if (prec < PrecUnary)
        t_->append(")");
    ...
    case kRegexpHaveMatch:
      ...
      AppendFormat(t_, "(?HaveMatch:%d)", re->match_id());
      break;
  }
...
static void AppendCCChar(std::string* t, Rune r) {
  ...
  if (r < 0x100) {
    AppendFormat(t, "\\x%02x", static_cast<int>(r));
    return;
  }
  AppendFormat(t, "\\x{%x}", static_cast<int>(r));
}
